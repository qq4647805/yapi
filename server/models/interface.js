...
class interfaceModel extends baseModel {
    ...
    getReqDependencies(id) {
        return this.model
            .find({
                "dependencies_query.id": id
            })
            .select("path title project_id")
            .exec()
    }
    ...
    listByCatid(catid, select) {
        select =
            select ||
            "_id title uid path method project_id catid edit_uid status add_time up_time index"
        return this.model
            .find({
                catid: catid
            })
