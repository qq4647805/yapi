...
class interfaceController extends baseController {
    ...
    /**
     * 获取项目分组
     * @interface /interface/getDependencies
     * @method GET
     * @category interface
     * @foldnumber 10
     * @param {Number}   id 接口id，不能为空
     * @returns {Object}
     * @example ./api/interface/getDependencies.json
     */
    async getReqDependencies(ctx) {
        let params = ctx.request.query
        if (!params.id) {
            return (ctx.body = yapi.commons.resReturn(
                null,
        ...
        try {
            let result = await this.Model.getReqDependencies(params.id)
            // console.log('result', result);
            if (!result) {
                return (ctx.body = yapi.commons.resReturn(
                    null,
    ...
    async up(ctx) {
        ...
        <p>详细改动日志: ${this.diffHTML(diffView)}</p></div>
            ...
            //通知关联接口
            let dependencies_res = await this.Model.getReqDependencies(
                interfaceData._id
            )
            dependencies_res.forEach(v => {
                let name = v.title + ":" + v.path
                yapi.commons.sendNotice(v.project_id, {
                    title: `${username} 更新了接口,可能会影响你的接口:${name} `,
                    content: `<html>
                        <head>
                        <style>
                        ${annotatedCss}
    ...
    async getCatMenu(ctx) {
        ...
        try {
            ...
            if (project.project_type === "private") {
                if (
                    (await this.checkAuth(project._id, "project", "edit")) !==
                    true
                ) {
                    return (ctx.body = yapi.commons.resReturn(
                        null,
    ...
    async getCustomField(ctx) {
        ...
        try {
            ...
            let groups = await this.groupModel.getcustomFieldName(
                customFieldName
            )
            if (groups.length === 0) {
                return (ctx.body = yapi.commons.resReturn(
                ...
                for (let j = 0; j < projects.length; j++) {
                    ...
                    if (inter.length > 0) {
                        ...
                        inter = inter.map((item, i) => {
                            ...
                            item.res_body = yapi.commons.json_parse(
                                item.res_body
                            )
