...
const dataTpl = {
    ...
    dependencies_query: { url: "", desc: "", id: 0 },
...
class InterfaceEditForm extends Component {
    ...
    initState(curdata) {
        ...
        if (
            curdata.dependencies_query &&
            curdata.dependencies_query.length === 0
        ) {
            delete curdata.dependencies_query
        }
            ...
            this.props.form.validateFields((err, values) => {
                ...
                if (!err) {
                    ...
                    values.req_query = values.req_query
                        ...
                        : []
                    values.dependencies_query = values.dependencies_query
                        ? values.dependencies_query.filter(
                              item => item.url !== ""
                          )
                        : []
                    ...
                    if (
                        ...
                        values.req_body_other = checkIsJsonSchema(
                            values.req_body_other
                        )
                        if (!values.req_body_other) {
                            return message.error(
                                "请求参数 json-schema 格式有误"
                            )
                        }
                    }
                    if (
    ...
    onChangeDefUrl = (data, index) => async e => {
        const url = e.target.value
        if (!url) {
            return
        }
        let match = url.match(/interface\/api\/(\d+)/)
        if (match) {
            let result = await axios.get("/api/interface/get?id=" + match[1])
            if (result.data.errcode === 0) {
                this.props.form.setFieldsValue({
                    ["dependencies_query[" + index + "].desc"]:
                        result.data.data.title + ":" + result.data.data.path,
                    ["dependencies_query[" + index + "].id"]: match[1]
                })
            }
        } else {
            this.props.form.setFieldsValue({
                ["dependencies_query[" + index + "].id"]: 0
            })
        }
    ...
    handleMockPreview = async () => {
        ...
        try {
            if (this.props.form.getFieldValue("res_body_is_json_schema")) {
                let schema = json5.parse(
                    this.props.form.getFieldValue("res_body")
                )
    ...
    render() {
        ...
        try {
            req_body_other = this.state.req_body_other
                ? JSON.stringify(
                      json5.parse(this.state.req_body_other),
                      null,
                      2
                  )
                : ""
        ...
        const dependenciesTpl = (data, index) => {
            return (
                <Row key={index} className="interface-edit-item-content">
                    ...
                    <Col span="10" className="interface-edit-item-content-col">
                        {getFieldDecorator(
                            "dependencies_query[" + index + "].desc",
                            {
                    ...
                    <Col span="1" className="interface-edit-item-content-col">
                        <Icon
                            ...
                            onClick={() =>
                                this.delParams(index, "dependencies_query")
                            }
                        />
                    </Col>
                </Row>
                ...
                <Row key={index} className="interface-edit-item-content">
                    ...
                    <Col span="1" className="interface-edit-item-content-col">
                        <Icon
                            ...
                            className="interface-edit-del-icon"
                            onClick={() =>
                                this.delParams(index, "req_body_form")
                            }
                        />
                    </Col>
                </Row>
        ...
        const dependenciesList = this.state.dependencies_query.map(
            (item, index) => {
                return dependenciesTpl(item, index)
            }
        )
            ...
            <Form onSubmit={this.handleSubmit}>
                ...
                <div className="panel-sub">
                    ...
                    >
                        <Col>
                            <EasyDragSort
                                data={() =>
                                    this.props.form.getFieldValue("req_query")
                                }
                    ...
                    <FormItem
                        className={
                            "interface-edit-item " +
                            this.state.hideTabs.req.headers
                        }
                    >
                        <Button
                            size="small"
                    ...
                    >
                        <Col>
                            <EasyDragSort
                                data={() =>
                                    this.props.form.getFieldValue("req_headers")
                                }
                    ...
                    <Row
                        className={
                            "interface-edit-item " +
                            (this.props.form.getFieldValue("req_body_type") ===
                            "json"
                                ? this.state.hideTabs.req.body
                                : "hide")
                        }
                    >
                        ...
                        {getFieldDecorator("req_body_is_json_schema", {
                            ...
                            initialValue:
                                this.state.req_body_is_json_schema ||
                                !projectMsg.is_json5
                        })(
                            <Switch
                                checkedChildren="开"
                                ...
                                <ReqBodySchema
                                    onChange={text => {
                                        ...
                                        if (
                                            ...
                                            EditFormContext.props.changeEditStatus(
                                                true
                                            )
                                        }
                                    }}
                    ...
                    {this.props.form.getFieldValue("req_body_type") ===
                        "file" && this.state.hideTabs.req.body !== "hide" ? (
                        <Row className="interface-edit-item">
                ...
                <h2 className="interface-title">
                    ...
                    {getFieldDecorator("res_body_is_json_schema", {
                        valuePropName: "checked",
                        initialValue:
                            this.state.res_body_is_json_schema ||
                            !projectMsg.is_json5
                    })(
                        <Switch
